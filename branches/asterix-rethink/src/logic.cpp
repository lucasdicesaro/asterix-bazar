...
{	
	...
	switch (ev.id)
	{
		int cantidad;
		case INIT:
			Logger::debug("Logic: on_event: INIT:");
			break;		
			
		case DO_LOOKUP:
			Logger::debug("Logic: on_event: DO_LOOKUP:");
			buffer = (char*) ev.tag;		
		...
		case DO_LOOKUP_FORWARD:
			Logger::debug("Logic: on_event: DO_LOOKUP_FORWARD:");			
			on_send_look_up_forward(ev.tag);
			break;								
			
		case DO_REPLY:
			Logger::debug("Logic: on_event: DO_REPLY:"); 
			on_send_replay(ev.tag);
			break;
						
		case DO_REPLY_FORWARD:
			Logger::debug("Logic: on_event: DO_REPLY_FORWARD:");
			on_send_replay_forward(ev.tag);			
			break;			
			
		case DO_BUY:
			Logger::debug("Logic: on_event: DO_BUY:");
			on_send_buy(ev.tag);			
			break;		
									
		case DO_BUY_REPLY_OK:
			Logger::debug("Logic: on_event: DO_BUY_REPLY_OK:"); 
			on_send_buy_reply_ok(ev.tag);
			break;			
			
		case DO_BUY_REPLY_ERR:
			Logger::debug("Logic: on_event: DO_BUY_REPLY_ERR:"); 
			on_send_buy_reply_err(ev.tag);
			break;						
			
		case CLIENT_MSG:
			Logger::debug("Logic: on_event: CLIENT_MSG:");
			on_client_msg(ev.tag);
			break;			

		case SET_CUR_PROD_COMPRA:
			Logger::debug("Logic: on_event: SET_CUR_PROD_COMPRA");
			prod = (const char*) ev.tag;
			set_cur_prod_compra(prod);
			break;	
			
		case SET_CUR_PROD_VENTA:
			Logger::debug("Logic: on_event: SET_CUR_PROD_VENTA");
			prod = (const char*) ev.tag;
			set_cur_prod_venta(prod);
			break;	
		
		case SET_STOCK_SAL:
			Logger::debug("Logic: on_event: SET_STOCK_SAL");
			buffer = (char*) ev.tag;		
			cantidad = atoi(buffer);		
		...
		case SET_STOCK_PESCADO:
			Logger::debug("Logic: on_event: SET_STOCK_PESCADO");
			buffer = (char*) ev.tag;		
			cantidad = atoi(buffer);		
		...
		case SET_STOCK_VERDURA:
			Logger::debug("Logic: on_event: SET_STOCK_VERDURA");
			buffer = (char*) ev.tag;		
			cantidad = atoi(buffer);		
...
void Logic::on_send_look_up_forward(const void* msg)
{
	...
	{						
		...
		{			
			...
			add_nodo(mensaje, nombre_nodo);					
			Logger::info("Reenviando el LOOKUP");					
		...
		{
			Logger::debug("Logic: on_send_look_up_forward: Se descarto el mensaje, por agotarse su hopcount");
	...
	{
		Logger::error("Logic: on_send_look_up_forward: mensaje es NULL en DO_LOOKUP_FORWARD. Se descarta");
	}
}
...
void Logic::on_send_replay(const void* msg)
{
	...
	if (mensaje != NULL)
	{
		Logger::info("Respondo con un REPLY");
	...
	{
		Logger::error("Logic: on_send_replay: mensaje es NULL en DO_REPLY_FORWARD. Operacion descatada");
	}		
}
...
void Logic::on_send_replay_forward(const void* msg)
{
	...
	if (mensaje != NULL)
	{
		Logger::info("Reenviando el REPLY");
		less_nodo(mensaje);		
	...
	{
		Logger::error("Logic: on_send_replay_forward: mensaje es NULL en DO_REPLY_FORWARD");
	}
}

void Logic::on_send_buy(const void* msg)
{
	Logger::debug("Logic::on_send_buy:");
	char logBuffer[BUFFER_SIZE];
...
{
	Logger::debug("Logic::on_send_buy_reply_ok:");
	char logBuffer[BUFFER_SIZE];
	Mensaje *mensaje_old = (Mensaje *)msg;		
...
{
	Logger::debug("Logic::on_send_buy_reply_err:");
	char logBuffer[BUFFER_SIZE];
	Mensaje *mensaje_old = (Mensaje *)msg;		
	Mensaje *mensaje = build_buy_reply_err_msg(mensaje_old);
	
	sprintf(logBuffer, "Le aviso al comprador [%s] que NO incremente lo suyo", mensaje_old->get_vendedor().c_str());
	Logger::info(logBuffer);
...
{
	Logger::debug("Logic: on_receive_replay:");
	char logBuffer[BUFFER_SIZE];
	...
	if (mensaje->count() <= 1)
	{
		...
		{
			Logger::debug("Logic: on_receive_replay: El ultimo nodo contiene mi nombre");
			memset(logBuffer, 0 , sizeof(logBuffer));
		...
		{
			Logger::warn("Logic: on_receive_replay: El ultimo nodo No contiene mi nombre: No deberia llegar aqui. Descarto el mensaje");			
		}
	}
	else
	{
		Logger::debug("Logic: on_codigo_replay: Sigue quedando mas de un nodo en el mensaje. Se reenvia");
...
{
	Logger::debug("Logic: on_receive_buy:");
	char logBuffer[BUFFER_SIZE];
...
{
	Logger::debug("Logic: on_receive_buy_reply_ok:");
	char logBuffer[BUFFER_SIZE];
...
{
	Logger::debug("Logic: on_receive_buy_reply_err:");
	char logBuffer[BUFFER_SIZE];
	sprintf(logBuffer, "El vendedor [%s] no pudo satisfacer mi pedido [%d unidades de %s]. No se modifica el stock. Operacion finalizada", mensaje->get_vendedor().c_str(), mensaje->get_cantidad(), mensaje->get_product_name().c_str());
	Logger::info(logBuffer);	
...
{
	Logger::debug("Logic: build_look_up_msg:");
	Mensaje *mensaje = new Mensaje();
...
Mensaje *Logic::build_reply_msg(Mensaje *mensaje_old)
{
	...
	Logger::debug("Logic: build_reply_msg:");
	if (mensaje_old != NULL)
	{
		mensaje = new Mensaje();
	...
	{
		Logger::error ("Logic: build_reply_msg: Mensaje recibido NULO");
	}
	return mensaje;
}
...
Mensaje *Logic::build_buy_msg(Mensaje *mensaje_old)
{
	char logBuffer[BUFFER_SIZE];
	Mensaje *mensaje = NULL;
	Logger::debug("Logic: build_buy_msg:");
	if (mensaje_old != NULL)
	{
		mensaje = new Mensaje();
	...
	{
		Logger::error ("Logic: build_buy_msg: Mensaje recibido NULO");
	}
	return mensaje;		
}
...
Mensaje *Logic::build_buy_reply_ok_msg(Mensaje *mensaje_old)
{
	char logBuffer[BUFFER_SIZE];
	Mensaje *mensaje = NULL;
	Logger::debug("Logic: build_buy_reply_ok_msg:");
	if (mensaje_old != NULL)
	{
		mensaje = new Mensaje();
	...
	{
		Logger::error ("Logic: build_buy_reply_ok_msg: Mensaje recibido NULO");
	}
	return mensaje;		
}
...
Mensaje *Logic::build_buy_reply_err_msg(Mensaje *mensaje_old)
{
	char logBuffer[BUFFER_SIZE];
	Mensaje *mensaje = NULL;
	Logger::debug("Logic: build_buy_reply_err_msg:");
	if (mensaje_old != NULL)
	{
		mensaje = new Mensaje();
	...
	{
		Logger::error ("Logic: build_buy_reply_err_msg: Mensaje recibido NULO");
	}
	return mensaje;		
}
...
{
	Logger::debug("Logic: less_hopcount:");
